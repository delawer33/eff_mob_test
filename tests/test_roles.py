...
@pytest.mark.asyncio
async def test_create_role_duplicate(client: AsyncClient, db_session: AsyncSession):
    existing = (await db_session.execute(select(Role).where(Role.name == "duplicate_role"))).scalar_one_or_none()
    if existing is None:
...
@pytest.mark.asyncio
async def test_get_role_success_and_not_found(client: AsyncClient, db_session: AsyncSession):
    r = (await db_session.execute(select(Role).where(Role.name == "support"))).scalar_one_or_none()
    if r is None:
        r = Role(name="support")
        db_session.add(r)
        await db_session.commit()
...
@pytest.mark.asyncio
async def test_update_role_success_and_duplicate_name(client: AsyncClient, db_session: AsyncSession):
    base = (await db_session.execute(select(Role).where(Role.name == "role_to_update"))).scalar_one_or_none()
    if base is None:
        base = Role(name="role_to_update")
        db_session.add(base)
        await db_session.commit()
        await db_session.refresh(base)
    other = (await db_session.execute(select(Role).where(Role.name == "taken_name"))).scalar_one_or_none()
    if other is None:
    ...
    app.dependency_overrides[get_current_user_admin] = _admin
    try:
        ok = await client.patch(f"/roles/{base.id}", json={"name": "updated_role_name"})
        assert ok.status_code == 200
        assert ok.json()["name"] == "updated_role_name"
        dup = await client.patch(f"/roles/{base.id}", json={"name": "taken_name"})
        assert dup.status_code == 400
        assert dup.json()["detail"] == "Another role with this name already exists"
    finally:
        app.dependency_overrides.pop(get_current_user_admin, None)


@pytest.mark.asyncio
async def test_delete_role_success_and_protected(client: AsyncClient, db_session: AsyncSession):
    del_role = (await db_session.execute(select(Role).where(Role.name == "temp_delete"))).scalar_one_or_none()
    if del_role is None:
    ...
    try:
        ...
        protected = (await db_session.execute(select(Role).where(Role.name == "admin"))).scalar_one()
