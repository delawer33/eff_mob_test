...
class TestAuth:
    ...
    async def test_register_user_already_exists(
        self, client: AsyncClient, user_payload_factory
    ):
    ...
    @pytest.mark.asyncio
    async def test_register_user_passwords_mismatch(
        self, client: AsyncClient, user_payload_factory
    ):
    ...
    async def test_login_user_success(
        self, client: AsyncClient, create_user_via_api
    ):
    ...
    async def test_assign_role_success(
        self, client: AsyncClient, db_session: AsyncSession, create_user_via_api
    ):
        user_json = await create_user_via_api(
            "roleuser", "roleuser@example.com"
        )
        ...
        try:
            req = {
                ...
                "user_identifier": "roleuser@example.com",
                "role_name": "admin",
            }
    ...
    async def test_update_user_success(
        ...
        resp = await client.patch(
            f"/auth/{u['id']}", json={"username": "newname"}
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    async def test_deactivate_user_success(
        ...
        u = await create_user_via_api(
            "to-deactivate", "to-deactivate@example.com"
        )
        login_data = {
