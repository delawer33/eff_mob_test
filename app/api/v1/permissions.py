...
from app.schemas.access_role_rule import (
    SAccessRoleRuleUpdate,
    SAccessRoleRuleResponse,
    SAccessRoleRuleCreate,
)
from app.models import User, Role, AccessRoleRule, BusinessElement
...
async def create_access_rule(
    ...
    try:
        ...
        be_id = select(BusinessElement).where(
            BusinessElement.id == access_rule_in.element_id
        )
        ...
        await db.rollback()
        logger.error(
            f"Database error in create_access_rule: {type(e).__name__}: {e}"
        )
        raise HTTPException(
        ...
        await db.rollback()
        logger.error(
            f"Unexpected error in create_access_rule: {type(e).__name__}: {e}"
        )
        raise HTTPException(
...
async def get_access_rules(
    ...
    element_id: Optional[int] = Query(
        None, description="Filter by business element ID"
    ),
    db: AsyncSession = Depends(get_async_db_session),
    ...
    try:
        ...
        if not rules:
            raise HTTPException(
                status_code=404,
                detail="No access rules found for the given filters",
            )
        ...
        await db.rollback()
        logger.error(
            f"Database error in get_access_rules: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="A database error occurred."
        )
        ...
        await db.rollback()
        logger.error(
            f"Unexpected error in get_access_rules: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="An internal server error occurred."
        )
...
async def get_business_elements(
    ...
    try:
        ...
        await db.rollback()
        logger.error(
            f"Database error in get_business_elements: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="A database error occurred."
        )
        ...
        await db.rollback()
        logger.error(
            f"Unexpected error in get_business_elements: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="An internal server error occurred."
        )
...
async def get_access_rule_by_id(
    ...
    try:
        result = await db.execute(
            select(AccessRoleRule).where(AccessRoleRule.id == rule_id)
        )
        rule = result.scalars().first()
        if not rule:
            raise HTTPException(
                status_code=404, detail="Access role rule not found"
            )
        ...
        await db.rollback()
        logger.error(
            f"Database error in get_access_rule_by_id: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="A database error occurred."
        )
        ...
        await db.rollback()
        logger.error(
            f"Unexpected error in get_access_rule_by_id: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="An internal server error occurred."
        )
...
async def update_access_rule_by_id(
    ...
    try:
        result = await db.execute(
            ...
            .options(selectinload(AccessRoleRule.role))
        )
        rule = result.scalars().first()
        if not rule:
        ...
        await db.rollback()
        logger.error(
            f"Database error in update_access_rule_by_id: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="A database error occurred."
        )
        ...
        await db.rollback()
        logger.error(
            f"Unexpected error in update_access_rule_by_id: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="An internal server error occurred."
        )
...
async def update_access_rule(
    ...
    try:
        ...
        await db.refresh(rule)
        logger.info(
            f"Updated access rule for role_id={role_id} and element_id={element_id}"
        )
        return rule
        ...
        await db.rollback()
        logger.error(
            f"Database error in update_access_rule: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="A database error occurred."
        )
        ...
        await db.rollback()
        logger.error(
            f"Unexpected error in update_access_rule: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="An internal server error occurred."
        )
...
@router.delete(
    "/access-rules/{rule_id}", status_code=status.HTTP_204_NO_CONTENT
)
async def delete_access_rule(
    ...
    try:
        ...
        await db.rollback()
        logger.error(
            f"Database error in delete_access_rule: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="A database error occurred."
        )
        ...
        await db.rollback()
        logger.error(
            f"Unexpected error in delete_access_rule: {type(e).__name__}: {e}"
        )
        raise HTTPException(
            status_code=500, detail="An internal server error occurred."
        )
