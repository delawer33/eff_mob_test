...
async def create_role(
    ...
    try:
        ...
        logger.info(f"Role created with ID: {new_role.id}, Name: {new_role.name}")
        return new_role
        ...
        await db.rollback()
        logger.error(f"Unexpected error in create_role: {type(e).__name__}: {e}")
        raise HTTPException(status_code=500, detail="An internal server error occurred.")
...
async def update_role(
    ...
    try:
        ...
        logger.info(f"Role {role_id} updated successfully. New Name: {role.name}")
        return role
        ...
        await db.rollback()
        logger.error(f"Unexpected error in update_role: {type(e).__name__}: {e}")
        raise HTTPException(status_code=500, detail="An internal server error occurred.")
...
async def delete_role(
    ...
    try:
        ...
        await db.rollback()
        logger.error(f"Unexpected error in delete_role: {type(e).__name__}: {e}")
        raise HTTPException(status_code=500, detail="An internal server error occurred.")
